...
function resetPuzzle() {
    ...
    console.log(JSON.stringify(posn), {emptyX, emptyY});
}
...
function paintToCanvas() {
 ...
 //   console.log(posn);
  ...
  //              console.log({pos, across, down})
                let block = ctx.getImageData(across*width/size, down*height/size, width/size, height/size)
                ctx2.putImageData(block, j*width/size, i*height/size);
            }
        }
// debugger;
    }, 1000);
...
function clickPuzzle(e) {   
 ...
 //   console.log({ emptyX, emptyY, clickX, clickY});
  ...
  //          console.log("same row");
            if (emptyX < clickX) {
                for (let col=emptyX; col < clickX; col++) {
                    n = clickY * size + col;
                    posn[n] = posn[n+1];
       //             console.log({n});
                }
